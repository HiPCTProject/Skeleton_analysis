...
def segment_feature_extraction(top, bottom):
   ...
   # pd = pandas.DataFrame(edges)
   # pd.to_csv("D:/Dropbox (UCL)/Hannah_Claire_vessel_comps/HREM/FADUS/Skeletons/VessVio_results/FADUS_run1_ordered_segments_edges.csv", encoding='utf-8', index=False)
...
def feature_input(
    ...
    ## outputs the graph info needed to make an amira graph.
    df=g.get_vertex_dataframe()
    df2=g.get_edge_dataframe()
    df.to_csv('D:/Dropbox (UCL)/Hannah_Claire_vessel_comps/HREM/Medulla_network/Consensus_optimisation/1st_iteration/STAPLE_Cropped_centre_run1_verts.csv', encoding='utf-8', index=False)
    df2.to_csv('D:/Dropbox (UCL)/Hannah_Claire_vessel_comps/HREM/Medulla_network/Consensus_optimisation/1st_iteration/STAPLE_Cropped_centre_run1_edge.csv', encoding='utf-8', index=False)
