...
for i=1:1%:height(genx_outliers)
    ...
    rad=ceil(genx_outliers.MeanRadius(i)/res); %% get the current radius of the segment to initalise the bounding box of the image
    clear seg
if genx_outliers.Identified_Graphs(i)==1 %% to identify which raw data label should be used, TODO could get rid of this if the whole volume was read in would be cleaner
    fprintf("subgraph1")
   ...
   rad_mult = 5;
    [answer,min_x,max_x,min_y,max_y,min_z,max_z,subvol] = getboundingbox(pt1,rad,rad_mult,raw_dat_cc1); %% this visualises the region and asks for user input before continuing
     if isnan(answer)
         skipped(i)=1;
...
elseif genx_outliers.Identified_Graphs(i)==4
    ...
    rad_mult = 5;
    [answer,min_x,max_x,min_y,max_y,min_z,max_z,subvol] = getboundingbox(pt1,rad,rad_mult,raw_dat_cc4); %% this visualises the region and asks for user input before continuing
     if isnan(answer)
         skipped(i)=1;
         final_rad_segments{i}=NaN;
...
elseif  genx_outliers.Identified_Graphs(i)==9
    ...
    rad_mult = 5;
    [answer,min_x,max_x,min_y,max_y,min_z,max_z,subvol] = getboundingbox(pt1,rad,rad_mult,raw_dat_cc9); %% this visualises the region and asks for user input before continuing
     if isnan(answer)
         skipped(i)=1;
         final_rad_segments{i}=NaN;
         continue   % should skip to the next line of genx_outlier if you decide it doesnt need fixing
...
% section to do the oblique slices for a particular segement       
for k=1:length(pt1)-1
  point1=[pt1{k}(1)-min_y,pt1{k}(2)-min_x,pt1{k}(3)-min_z];  %first point corrected position within the subvolume
  point2=[pt1{k+1}(1)-min_y,pt1{k+1}(2)-min_x,pt1{k+1}(3)-min_z];%second point corrected position within the subvolume
  normal=pt1{k}-pt1{k+1}; %% normal between the two points

  [test_reslice,xtest,ytest,ztest]=obliqueslice(subvol,point1,normal); 

 %% For visualising slices every 10th uncomment the below This code is super annoying and should be updated to make it simpler 
if k==1;
    vizualisation(subvol,xtest,ytest,ztest,k,point1,point2)
elseif rem(k,10)==0
    vizualisation(subvol,xtest,ytest,ztest,k,point1,point2)
end
%% This block of code should ideally be put inside the visualisation function it is for looking at the 2D slices but it very annoying. 
figure 
